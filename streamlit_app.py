...
2024-12,1.93,1.98
"""
# PDF 파일 ('climatechange_05.pdf')에서 추출한 '주요 어종별 양식 생산량' 데이터 (15페이지)
...
"""
# CSV 파일 ('퀴즈(응답) - 설문지 응답 시트1.csv')에서 추출한 사용자 설문조사 데이터
...
st.markdown("""
최근 기후변화와 더불어 해수온 상승 문제가 전 세계적으로 주목받고 있습니다. 우리 주변에서도 이 문제에 대한 관심이 점차 높아지고 있으며, 청소년 세대 역시 그 심각성을 인식하고 있습니다. 미림마이스터고 학생들을 대상으로 진행한 설문조사 결과, 다수의 학생들이 해수온 상승을 중요한 환경 문제로 인식하고 있었습니다. 이러한 결과는 청소년 스스로 기후위기 문제에 대해 탐구하고 해결책을 고민할 필요성을 보여줍니다. 따라서 본 보고서에서는 해수온 상승의 원인과 그로 인한 문제, 그리고 이를 해결할 수 있는 방안을 제시하고자 합니다.
...
with st.container(border=True):
    ...
    st.markdown("""
    <p>해수온 상승 그래프는 지구 온난화로 인해 해수면 온도가 지속적으로 상승하는 추세를 시각적으로 보여줍니다. 해수온이 상승하면 해수면이 상승하면서 해안 도시가 침수되는 사례도 발생하게 됩니다.</p>
    """, unsafe_allow_html=True)
    
    st.markdown(f"**해수온 상승은 우리의 삶에 다양한 위협을 가져오고 있습니다.** 바닷물이 따뜻해지면서 태풍의 위력이 강해지고 해양 생태계가 무너져 어획량이 감소하며, 결국 우리의 식량 안보와 경제에도 큰 타격을 주기 때문입니다. 실제로 2016년 호주 대보초에서는 해수온이 급격히 오르면서 전체 산호의 절반 가까이가 백화 현상으로 소멸했고, 한반도 주변 해역에서도 난류성 어종이 늘어 전통적인 어획 구조가 흔들리고 있습니다. 이처럼 해수온 상승은 단순히 바다의 문제가 아니라 인간 사회 전반에 영향을 미치는 심각한 위기이므로, 적극적인 대응과 해결 노력이 필요합니다.", unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    st.markdown("""
    <p>최근 기후변화와 더불어 해수온 상승 문제가 전 세계적으로 주목받고 있습니다. 이 보고서는 해수온 상승의 원인과 그로 인한 문제, 그리고 이를 해결할 수 있는 방안을 제시합니다.</p>
    """, unsafe_allow_html=True)
    st.markdown("""
    **해수온 상승의 가장 큰 원인은 인간 활동으로 인한 온실가스 배출입니다.** 산업화 이후 화석연료 사용이 급격히 늘어나면서 대기 중 이산화탄소 농도가 증가했고, 이로 인해 지구 온난화가 심화되며 바다 역시 빠르게 따뜻해지고 있습니다. 해수온 상승은 단순한 자연 현상이 아니라, 인간 활동이 직접적으로 초래한 지구적 위기라 할 수 있습니다.
    """, unsafe_allow_html=True)
...
st.header("4. 데이터 출처")
st.markdown("""
- **전 지구 기온 이상값**: Berkeley Earth Land/Ocean Temperature Record
- **주요 어종별 양식 생산량**: 해양수산분야 기후변화 영향 브리핑 북 2025 (`climatechange_05.pdf`, 15쪽)
- **학생 설문조사 데이터**: 미림마이스터고 학생 설문조사 (`퀴즈(응답) - 설문지 응답 시트1.csv`)
""")
