...
for region in df_filtered['지역'].unique():
    ...
    fig_ecosystem.add_trace(go.Bar(x=df_r['date'], y=df_r['산호초 백화(%)'], name=f"{region} 산호초 백화"))
    fig_ecosystem.add_trace(go.Bar(x=df_r['date'], y=df_r['어류 이동 변화(%)'], name=f"{region} 어류 이동"))
    fig_ecosystem.add_trace(go.Bar(x=df_r['date'], y=df_r['침수 위험 지수'], name=f"{region} 침수 위험"))
